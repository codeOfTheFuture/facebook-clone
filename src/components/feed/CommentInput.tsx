...
const CommentInput: FC<Props> = ({ postId }) => {
  ...
  const [imageToComment, setImageToComment] = useState<string | null>(null);
  const [comment, setComment] = useState("");
  const filePickerRef = useRef<HTMLInputElement>(null);

  const addImageToComment = (e: ChangeEvent<HTMLInputElement>): void => {
  ...
  const sendComment = async (e: FormEvent): Promise<void> => {
    e.preventDefault();
